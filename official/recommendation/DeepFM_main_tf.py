...
from official.recommendation.DeepFM_mode_tf import model_fn
...
def convert_keras_to_estimator(model_fn, num_gpus, model_dir, model_params):
    ...
    estimator = tf.estimator.Estimator(model_fn=model_fn, params=model_params, model_dir=model_dir, config=run_config)
...
def run_ncf(_):
    ...
    estimator = convert_keras_to_estimator(model_fn, num_gpus, FLAGS.model_dir, params)
    ...
    for cycle_index in range(total_training_cycle):
        ...
        with open("./result.txt", "a") as f:
            f.write("Iteration {}: HR = {:.4f}, NDCG = {:.4f}".format(
                cycle_index + 1, hr, ndcg))
            f.write("\n")
